...
create index if not exists complaints_member_created_at_idx
  on public.complaints (member_id, created_at desc);

create index if not exists complaints_status_priority_idx
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'Members can read own complaints'
  ) then
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'complaints'
      and policyname = 'Members can submit own complaints'
  ) then
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'complaints'
      and policyname = 'Staff can manage complaints'
  ) then
