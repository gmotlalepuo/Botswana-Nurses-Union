  ...
  return (
    <section className="mb-6 rounded-lg border bg-white p-5 shadow-sm">
      <div>
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            Upload CSV or Excel rows matched by National ID / Omang. One combined payment is recorded per member and payment month.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 rounded-lg border bg-muted/30 p-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">The file includes members with approved monthly deductions and prefills their expected combined totals.</p>
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_13rem_auto] md:items-end">
          ...
          <a
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border bg-white px-4 py-2.5 text-sm font-semibold text-primary hover:bg-muted"
            href={`/api/csr/payments/template?council=${encodeURIComponent(council)}&paymentMonth=${encodeURIComponent(paymentMonth)}`}
          >
